...
Level *LevelParser::parseLevel(const char *levelFile) {
  ...
  // Parse the tilesets.
  for (tinyxml2::XMLElement *e = pRoot->FirstChildElement(); e != nullptr;
  ...
  for (tinyxml2::XMLElement *e = pRoot->FirstChildElement(); e != nullptr;
    ...
    if (e->Value() == std::string("objectgroup") ||
      ...
      if (e->FirstChildElement()->Value() == std::string("object")) {
        ...
        parseTileLayer(e, pLevel->getLayers(), pLevel->getTitlesets(), pLevel->getCollisionLayers());
      }
    }
  }
  ...
  for (tinyxml2::XMLElement *e = pRoot->FirstChildElement(); e != nullptr;
    ...
    if (e->Value() == std::string("properties")) {
      for (tinyxml2::XMLElement *pProperty = e->FirstChildElement();
           pProperty != nullptr; pProperty = pProperty->NextSiblingElement()) {
...
void LevelParser::parseTilesets(tinyxml2::XMLElement *pTilesetRoot,
  ...
  std::string sourcePath = std::string("assets/") + pTilesetRoot->FirstChildElement()->Attribute("source");
...
void LevelParser::parseTileLayer(tinyxml2::XMLElement *pTileElement,
  ...
  for (tinyxml2::XMLElement *e = pTileElement->FirstChildElement();
    ...
    if (e->Value() == std::string("properties")) {
      for(tinyxml2::XMLElement *property = e->FirstChildElement(); property != nullptr; property = property->NextSiblingElement()) {
  ...
  for (tinyxml2::XMLNode *e = pDataNode->FirstChild(); e != nullptr;
       e = e->NextSiblingElement()) {
...
void LevelParser::parseObjectLayer(tinyxml2::XMLElement *pObjectElement,
  ...
  for (tinyxml2::XMLElement *e = pObjectElement->FirstChildElement();
    ...
    if (e->Value() == std::string("object")) {
      ...
      pGameObject->load(std::make_unique<LoaderParams>(
